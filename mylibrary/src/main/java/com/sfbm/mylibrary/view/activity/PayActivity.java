...
import com.sfbm.mylibrary.bean.MyALipayBean;
import com.sfbm.mylibrary.bean.MyWeChatPayBean;
import com.sfbm.mylibrary.util.MyToastUtils;
...
public class PayActivity extends AppCompatActivity {
    ...
    private MyWeChatPayBean weChatPayBean;
    private String payParams;
    private MyALipayBean aLipayBean;
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG:
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        Intent intent=new Intent(PayActivity.this,PaymentSuccessActivity.class);
                        startActivity(intent);
//                        startActivity(new Intent(RechargeActivity.this, MyWalletActivity.class));
                        finish();
                    } else {
                        // 该笔订单真实的支付结果，需要依赖服务端的异步通知。
                        Intent intent=new Intent(PayActivity.this,OrderTimeoutActivity.class);
                        startActivity(intent);
                        finish();
...
//                    Toast.makeText(PayActivity.this, "微信", Toast.LENGTH_SHORT).show();
...
//                    }
                    ...
                    if (conn.getResponseCode() >= 400) {
                        ...
                        if(channelId != null && channelId.equals("wxpay_app") ) {
                            //微信
                            weChatPayBean = gson.fromJson(stringBuilder.toString(), MyWeChatPayBean.class);
                            //判断微信返回参数是否成功
                            if(weChatPayBean.getCode() == 0) {
                                payParams = weChatPayBean.getData().getPayParams();
                                startWxPay(payParams);
                            }else {
                                MyToastUtils.showLong(context, "服务器异常");
                            }
                        }else if (channelId != null && channelId.equals("alipay_mobile") ) {
                            //支付宝
                            aLipayBean = gson.fromJson(stringBuilder.toString(), MyALipayBean.class);
                            //判断支付宝返回参数是否成功
    ...
    private void showResponse(final String response) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                //进行UI操作,将结果显示到界面
                //Toast.makeText(CashierDeskActivity.this, "UI", Toast.LENGTH_SHORT).show();
